...
node("nodejs"){
    ...
    stage('Clone Git Repo') {
        catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
            git branch: 'main', credentialsId: 'github-goitProjects', url: 'git@github.com:goitProjects/questify_backend_v2.git'
        }
    }
    ...
    stage('Copy files'){
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                sh '''cp /root/keys/index.ts /root/slave_jenkins/workspace/Build-Questify-Backend/src/gcp-config/index.ts'''
                sh "cp /root/keys/keys.json /root/slave_jenkins/workspace/Build-Questify-Backend/src/gcp-config/keys.json"
                sh "cp /root/keys/questify-backend/.env /root/slave_jenkins/workspace/Build-Questify-Backend/.env"
            }
        }
    }
    ...
    stage('Build'){
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                sh "npm i && npm i -g typescript@4.0.5 && tsc"
            }
        }
    }
    ...
    stage('Deploy') {
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                //sent files to  questify-backend.goit.global
                sh "scp -r ./* ${env.sshUserAndHost}:/root/questify-backend"
            }
        }
    }
